...
> = async ({
  ...
  const signAndSendTransaction = async ({
    ...
    if (!contractId) {
      ...
      const stringifiedParam = JSON.stringify([
        { receiverId, signerId, actions },
      ]);
  ...
  const verifyOwner = async (): Promise<void> => {
    throw new Error(
      `The verifyOwner method is not supported by ${metadata.name}`
    );
  };
  ...
  const getAccounts = async (): Promise<Array<MintbaseWalletAccount>> => {
    const accountId = state.wallet.getAccountId();
    const account = state.wallet.account();
  ...
  const signIn = async (): Promise<Array<MintbaseWalletAccount>> => {
  ...
  const transformTransactions = async (
    ...
    const account = !!state && state.wallet.account();
